...
5. **Web costs us the strongest fraud control that exists.** No Play Integrity, no App Attest — at exactly the moment rewards became voucher-scale. Compensate with per-checkpoint tokens, questions asked at server-chosen moments during the video, a segment-log cross-check on our own HLS origin that needs no client cooperation, holdback by trust tier, and daily earn caps. Sign-in is email and password. Keep a Capacitor wrap as the escape hatch.

6. **Australia and Indonesia are two separate economies.** Australia is the primary market (English, AUD); Indonesia (IDR, Bahasa available) is the proving ground. Each has its own points, rates, tax and policy, and nothing crosses between them. The wall is enforced in the database, the ledger and Cerbos, not only in the UI. See [TASKS.md](TASKS.md), decision F2.
