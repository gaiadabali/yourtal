...
- [x] **1.5 The principal comes from the session, never from headers** · needs: 1.4 — ✅ 2026-09-26 4916e31
  ...
  - [x] 1.5.b The principal carries:
    ...
    Cerbos policies compare the principal's region with the resource's region on every resource (the F2 hard walls). (requested by C for 9.1) `pnpm staff:add <email> <role>` writes `identity.staff_role`. — ✅ 2026-09-26: `AsyncPrincipalResolver` overlays jurisdiction/ageBand/isSuspended/businessRoles from `identity.user_profile` + `business.business_members` once a profile row exists (falls back to header-derived values otherwise, so every fixture with no real profile is unaffected); `identity.staff_role` folds into `roles` either way. `PdpGuard` auto-injects a tenant-scoped resource's `region` via `BusinessRegionLookup`; the `f2-region-wall` DENY (`has(R.attr.region) && P.attr.jurisdiction != R.attr.region`, scoped to `user`/`business_user` — staff and `store_device` are exempt by design) is now in business/billing/kyb_document/team/listing/campaign/voucher_batch/redemption/report/campaign_view. New `RegionWallSuite` proves it; native Cerbos suite 509/509. `pnpm check` green; `apps/api` 55 files/340 tests green against the (restarted) shared sidecar.
  ...
  - [x] 1.5.e HTTP hardening: — ✅ 2026-09-26 e5756f3. `main.ts` sets Fastify `trustProxy: true` (every real deployment runs behind Helios's nginx, or every per-IP limit collapses into one shared bucket). The one cookie this API sets (`yt_signup_blocked`) gets `; Secure` except in dev, matching `apps/web/lib/api/session-cookies.ts`'s own gate. Session lifetimes were wrong hard-coded constants (`SESSION_IDLE_TTL_MS` was 30 MINUTES, not F12's 30-day sliding number) — now three config values defaulted to F12's actual numbers (consumer 30d idle / 90d absolute, staff 12h absolute), read by `SessionService`. A staff account signs in through the same `AuthService` a consumer does (`pnpm staff:add` only grants an existing account a role row), so `AuthService.issueSession()` checks `identity.staff_role` fresh at every mint rather than trusting the route. Verified against real Postgres, driving time through the explicit `now` parameter rather than waiting real days: the sliding window survives a touch at +45d from issuance (only +25d from the last touch) but refuses one truly idle past 30d; the 90-day absolute ceiling refuses an actively-touched session at +95d; a staff session's 12h ceiling refuses at +13h where a consumer session issued at the same instant is still valid; logging in for a userId holding `identity.staff_role` issues a session whose `absoluteExpiresAt` is ~12h out.
    - Fastify `trustProxy`;
  ...
  - [x] 1.5.h (F30) A protected route called with no session returns 401, not 403; a signed-in principal that Cerbos refuses still gets 403. Update 1.5.g's check test to assert 401. — ✅ 2026-09-26 4916e31: `PdpGuard` maps an anonymous principal's Cerbos DENY to 401/`no_session` instead of the usual 403/`forbidden`; only on `type: "forbidden"` — `pdp_unavailable`/`pdp_protocol_error` stay 503 regardless of who is asking, and Open Viewing's anonymous ALLOW path (`campaign_view.yaml`) is untouched since this only fires once Cerbos has already said no. Updated every existing "refuses an anonymous caller" test that expected 403 (`app.boot.test.ts` x2, `me.controller.test.ts` x2, `wallet.controller.test.ts`) to expect 401/`no_session`; every SIGNED-IN-but-refused case (`team-member.routes.boot.test.ts`, `store-listing.routes.boot.test.ts`, the direct-principal settlement/material-decrease tests) stays 403, unchanged. Two new `pdp.guard.test.ts` unit tests pin both the 401 branch and that it does not touch the 503 infra-failure path. `session-and-region-wall.check.e2e.test.ts`'s forged-header case now asserts 401; its signed-in ID-viewer-vs-AU-campaign case stays 403 — fixing along the way a real cross-file flake this check's own region flip caused in `campaign.controller.e2e.test.ts` (one shared test database per run, not one per file; reverted in a `finally`). No web code special-cases 401 vs 403 today (grepped), so nothing there needed changing. `apps/api`: 65 files/380 tests green (.env sourced, this worktree's own Cerbos); Cerbos native suite 509/509; `pnpm check` green.
...
- [x] **2.2 Continuous deploy from main** · needs: 2.1 — ✅ 2026-09-26 c361aa1
...
- [x] **3.1 Fix the CSS pipeline first (half a day; fixes dialogs and badges immediately)** · needs: 0.2.b — ✅ 2026-09-25 4562a65
...
- [x] **3.5 Video primitives and shells** · needs: 3.4 — ✅ 2026-09-26 cad8f7e
  ...
  - [x] 3.5.d Move `(app)/page.tsx` to `(app)/home/page.tsx`. In the same merge, add these to `route-redirects.ts`: `/` → `/home` when signed in, and `/` → `/au` when signed out, until 11.1.a.
...
- [x] **3.6 Brand, gallery and visual tests** · needs: 3.4 — ✅ 2026-09-26 1f00762
  ...
  - [x] 3.6.d **Check:** `pnpm test:visual` passes locally and in CI.
...
- [x] **4.7 Burn saga: points become a voucher exactly once** · needs: 4.3, 4.5 — ✅ 2026-09-26 8c29713
  ...
  - [x] 4.7.d **Check:** passed live on main 8c29713: `pnpm test:checkout-live` (both Go services on one test database, `LEDGER_MODE=live`, the spec kills and restarts the voucher service), 5/5. The run found that an unreachable voucher service after the burn threw out of the saga and stopped recovery for every saga, and that an unreachable ledger read as "no burn" and released a spent voucher; both fixed (3b93b06, `recover-sagas.test.ts`).
...
- [x] **5.1 A watch session that cannot be farmed** · needs: 1.5, 7.2.c (fake ok) — ✅ 2026-09-26 acbdf73
  - [x] 5.1.a Cumulative budget: total accepted seconds ≤ (now − `startedAt`) + one tolerance. Take a per-session row lock (or compare-and-set on `last_progress_at`), anchor each span to the last accepted end, and rate-limit `progress` (EW-01). Add burst and parallel regression tests. — `judgeProgressReport` (watch-progress-report.ts) rewritten around the session's cumulative covered seconds vs. wall-clock elapsed since `startedAt`, one tolerance total; `DrizzleWatchSessionRepository.recordProgress` runs it inside a `SELECT … FOR UPDATE` on the session row, so concurrent reports serialise. `@RateLimit` added to `progress`. Burst (600 sequential) and parallel (30 concurrent) regression tests both as pure-function unit tests and as real HTTP e2e (5.1.e).
  - [x] 5.1.b Coverage is keyed by (user, campaign, terms version), not by session.
  ...
  - [x] 5.1.c Completion:
    ...
    — `markCompleted` now returns whether it matched (`.returning()`), and `grantReward` is only called on a match. Every duration/reward figure on the earning path (progress budget, checkpoint schedule, completion target, points paid) reads `CampaignRepository.termsVersionDetails(campaignId, session.termsVersion)` — the FROZEN row, never the campaign's current config. `progress()` also checks `isLive` (a gap found after first marking this done — fixed, real HTTP regression test added). `seed/watch.ts` forces every campaign's `duration_seconds` (campaign row AND terms_version) to 30, and — found by the same seed run — rescales `campaign.chapter.start_seconds` and clamps `teaser_start_seconds`, since `campaignSchema`'s own refine on chapter-before-duration was silently dropping every long_form campaign from `listVisible` otherwise. `time-remap.ts` and its 3 web call sites deleted (real/virtual seconds are now the same number).
  - [x] 5.1.d Signed segments (EW-18). Session start returns a per-session manifest URL from `signedSegmentUrl()` in `@yourtal/media` (requested by B; C exports it in 7.2.c). Until then, B calls a local stub with the same signature that returns an unsigned URL. nginx checks the signature (2.1.c). A records the served segments and exposes `deliveryCoverage(sessionId)` (10.4.c), which B's completion calls to flag gaps. — Done as scoped ("until then"/behind-an-interface, both fake ok): `start()` returns `manifestUrl` from a local unsigned stub (`media/segment-url-stub.ts`, same call shape `mintSegmentUrl` will have); `complete()` calls a `DeliveryCoverageReader` port whose only implementation today is a stub returning `"unknown"` (`delivery-coverage.ts`) — 10.4.c isn't built yet. Both are already tracked as `(requested by B)`/"B's completion calls it" under 7.2.c and 10.4.c respectively; swapping either in is a two-line change in `watch.module.ts`, not a redesign.
  - [x] 5.1.e **Check:**
    ...
    — real HTTP round trips, real Postgres, real (fake-mode) ledger client: `apps/api/src/modules/watch/watch-earn-journey.e2e.test.ts`. 600 sequential progress POSTs measured against actual wall-clock elapsed (asserted `coveredSeconds ≤ elapsed + tolerance`, not a hard 3 s, since CI speed varies — the invariant proved is the one that matters); a 30-parallel burst bounded the same way; park/resume verified (coverage preserved across the round trip); `already_earned` verified after a real completion. All passing — see this task's Check evidence in the report.
- [x] **5.2 Questions during the video, served and scored on the server (F10)** · needs: 5.1 — ✅ 2026-09-26 acbdf73
  - [x] 5.2.a One count everywhere (EW-21): `questionsAskedFor` (1.1.f) is used by `checkpoint.controller` (replacing `campaign.questionCount`), Studio validation (7.3) and the seed. — `checkpoint.controller.ts` and `watch.controller.ts` (completion's `questionsAnswered` gate) both compute the schedule from `questionsAskedFor(termsVersion.durationSeconds)`; Studio validation (7.3) and the seed's own bank-size rule were already on `questionsAskedFor`/`requiredBankSize` (question-bank.ts) — not changed here, confirmed still the only definition in use.
...
- [x] **5.3 Completion grants the reward (EW-06)** · needs: 5.2, 4.4 (fake ok) — ✅ 2026-09-26 acbdf73
  - [x] 5.3.a On completion, call `ledger-client.grantReward` with `ExternalRef = sessionId` and the signed attestation including asked and correct (4.4.c). The response is the pending points and their unlock date, and the UI shows exactly that figure. — `WatchController.complete()`: for an earning session, `signRewardAttestation` (4.4.c, already built) signs session/user/campaign/terms/completedAt/asked/correct; `pointsForCompletion` (new, `reward-for-completion.ts`) computes base + bonus (bonus only on a perfect score, per `base_plus_accuracy_bonus`); `grantReward` is called with `idempotencyKey = watch-grant:{sessionId}` and the session's own hold id. Response: `{granted, pendingPoints, unlockAt}` on success, `{granted:false, reason}` for a non-earning session or a failed grant (session still completes; the grant is not retried automatically — noted, not built). A `WATCH_COMPLETION_HOOK` (optional, no-op by default) fires right after a successful grant — 5.5.d's own ask: "make completion's result easy to hook"; see `watch-completion-hook.ts`'s header for exactly how to bind the streak module's real listener (a provider override in `app.module.ts` or a shared module — not built here, `modules/me` is out of this session's area).
  - [x] 5.3.b **Check:** in an HTTP round trip, start → progress → answer all → complete produces a ledger pending entry for the terms' points. A second complete returns the same result with no second grant. — `watch-earn-journey.e2e.test.ts`, against the real `LedgerInternalClient` (fake mode): full flow produces `granted:true, pendingPoints = REWARD_POINTS`, and `ledger.balance(userId).pending` shows the same points with `unlockAt` in the future (a fresh account is trust tier 0 — F12's 72 h holdback — so this is genuinely pending). Replaying the SAME `Idempotency-Key` returns the identical response (`@Idempotent`'s own replay, never re-executing the handler); a genuinely second attempt (a fresh key) gets 403 from `judgeCompletion`'s own terminal-state refusal, never a second grant either way — balance unchanged after both.
...
- [ ] **5.6 Earning proven against the real ledger and staging** · needs: 5.3, 2.1 — 🔄 slot 2
  - [x] 5.6.a Session start returns a manifest URL signed with `shared/media-auth/hls-token.ts`, the token `/api/internal/hls-auth` already verifies for nginx (2.1.c). Retire `segment-url-stub.ts`; swap to `@yourtal/media`'s `mintSegmentUrl` only if 7.2.c exports one with the same scheme. — `packages/media/src/signed-segment-url.ts` exists but is not exported and signs a different scheme (opaque per-segment `?t=` token, not nginx's path-embedded session token), so this calls `hls-token.ts`'s `signHlsPath` directly, per the task's own fallback. `mint-manifest-url.ts` returns a relative `/media/hls/...` path (nginx fronts it on the same origin `/api/` already uses), signed with `HLS_SIGNING_SECRET` (same secret `HlsAuthController` verifies with), expiring at the campaign's duration plus the 24h the idempotent `start` response can be replayed for. `segment-url-stub.ts` deleted. Unit test `mint-manifest-url.test.ts`: a minted URL passes `verifyHlsUri`; a tampered session id and a forged secret are refused; it expires correctly. `pnpm --filter @yourtal/api test` 71/71 files, 428/428 tests green (real HTTP e2e included); `pnpm check` green. Merged 0693f67 (after an unrelated regression fix, 73767ec — see Log).
  - [x] 5.6.b **Check:** 5.3.b's round trip with `LEDGER_MODE=http` against the running Go ledger service: the pending entry is in the ledger's own tables for the terms' points, and a second complete adds nothing. (5.3.b ran against the fake ledger.) — unblocked once F36's rename (`33a7603`) landed and the coordinator repointed `yourtal_s2`'s `atlas_schema_revisions` rows; `pnpm db:migrate` then reported nothing to run. Built `services/ledger`'s real binary (`go build ./cmd/ledger`) and ran it on 26322 against `yourtal_s2` (`LEDGER_DATABASE_URL`, this slot's `LEDGER_SERVICE_SECRET`/`REWARD_ATTESTATION_SECRET`); started `apps/api` on 26321 (worktree `.env` sourced, `LEDGER_MODE=live`, matching the `Env` schema's actual enum value — the task text said "http", the code calls it "live"). Seeded campaigns' `reward_config` points at `platform.ledger_fake_allocation`, invisible to the real ledger's own tables, so drove a one-off campaign wired to a REAL `ledger.allocation` (via `HttpLedgerClient.purchasePoints`, script discarded after use, never committed). Real HTTP round trip: registered + logged in a real ID-region user (`POST /api/auth/register` then `/login`), `POST /api/watch/sessions` (real `hold` against the real allocation, `nonEarning:false`), backdated `watch.session.started_at` by 35s (same technique `watch-earn-journey.e2e.test.ts`'s `backdateSessionStart` uses, since the 30s campaign can't be legitimately covered in zero elapsed time), `POST .../progress` 0→30 (`accepted:true, coveredSeconds:30`), `POST .../complete` → `{granted:true, pendingPoints:200, unlockAt: 2026-09-29}` — `SELECT * FROM ledger.grant` shows exactly one `watch_completed` row for 200 points, matching the terms. A second `complete` with a FRESH idempotency key → 403 "session is completed and cannot be completed"; grant count unchanged at 1. `pnpm --filter @yourtal/api test` run 3× back to back post-migrate: 72/72 files, 430/430 tests, stable every time.
  - [ ] 5.6.c **Check:** on staging, a signed manifest URL from session start loads through nginx, and the same URL without its token returns 403. — ⛔ not media, not deploy: staging redeployed past `0693f67` (confirmed `/api/health` revision `e367574`, per the coordinator). Registered two real demo accounts (AU and ID) through the public API, started real sessions against all 4 seeded staging campaigns (`00000000-0000-4000-9000-0000000001{01,02}` AU, `0201/0202` ID) — every one signs and returns a `/media/hls/...` manifest URL. The AUTH HALF is proven: fetching the real signed URL through `https://yourtal.gaiada.com` reaches nginx's origin (a genuine S3-level 404 `NoSuchKey` — signature accepted, request passed `auth_request`), while a tampered token and a token with the wrong session id both get a clean 403 straight from nginx. The CONTENT half is not provable: every staging campaign's `hlsUrl` points at the same `attention-30s/index.m3u8` fixture object, and Helios's real `yourtal-media` bucket does not have it (`NoSuchKey`, `Resource: /yourtal-media/hls/attention-30s/index.m3u8`) — no campaign on staging has playable media, so no signed URL can be shown to actually "load" content, only to pass or fail the signature check. Re-run once the media fixture is uploaded to staging's bucket (2.3-family scope, not this task's).
  - [x] 5.6.d Fix the integration regression from `acbdf73`: two `ledger-client.contract.spec.ts` tests (campaignSpend/returnGrant; reward grant to pending) fail since 5.1–5.3 merged (CI run 36252790816). Integration green on main again. — the ledger-spec fix (73767ec) exposed a second, chained bug: its `rewardedCampaign()` fixture inserted a `live` campaign with no `campaign.video_source` and no `campaign.chapter` rows (required for `kind='long_form'`), so `campaignSchema` could never parse it and it sat `live` forever in the shared `with-test-db.mjs` database. `session-and-region-wall.check.e2e.test.ts`'s own `ORDER BY random() LIMIT 1` control read could land on it and 404 instead of 200 — verified directly, not assumed (traced the exact query, the exact missing columns, and the missing cleanup). Fixed both: `ledger-client.contract.spec.ts`'s fixture now has a video_source and one chapter; `session-and-region-wall.check.e2e.test.ts` no longer picks a random row at all — it creates, flips and deletes its own dedicated campaign. Merged 9009b2b. `pnpm --filter @yourtal/api test` run 3× back to back: 72/72 files, 430/430 tests, stable every time. That commit's own Integration run then surfaced a THIRD instance of the identical bug, in Go: `services/ledger/internal/reward/campaign_fixture_test.go`'s `newLiveCampaign` and `internal/api/api_test.go`'s `TestTheLedgerRoutesEndToEnd` run the exact same `LEFT JOIN … WHERE r.campaign_id IS NULL` query and `t.Skip()` when it finds nothing — which the integration gate counts as a hard failure ("A Postgres-backed test skipped… its result means nothing"), not a pass. Same fix: both now `INSERT`/own a fresh `campaign.campaigns` row (`gen_random_uuid()`, live from the start) and delete it in `t.Cleanup` (chapter/terms_version/reward_config cascade off it); neither package reads through `campaignSchema` so no video_source/chapter row is needed there. `go build`/`go vet` clean; `go test -count=1 -p 1 ./...` via `with-test-db.mjs` — every package `ok`, zero skips. Merged 78d90e4. Integration run for 78d90e4 (CI run 36257952538) completed green — `gh run list --repo gaiadabali/yourtal --workflow=integration.yml --branch main` shows `completed success` for this commit. Integration on main is clean again.
...
- [ ] **6.1 One i18n system, English by default** · needs: 3.4 — 🔄 slot 3
  ...
  - [ ] 6.1.b Display language is independent of region, stored in `profile.display_locale` and the `yt_locale` cookie. Add a language switch on Me and in the public header. `i18n/request.ts` reads only `yt_locale` and `yt_region` (1.7.b). — Me half done (slot 3, agent C): `MeLanguageSection`/`MeProfileSection` submit straight to the existing `updateMeAction` (`lib/api/actions.ts`, untouched); verified with a real browser session — the form POST carries `displayLocale`, `PATCH /api/me` returns the new value, and `yt_locale` is set to it. The public-header half and `i18n/request.ts`'s own switch to reading `yt_locale`/`yt_region` are still open (agent A's side); until that lands, Me's own catalogue keeps rendering from the old region-cookie-derived locale even after a successful switch — re-run 6.1.e once `i18n/request.ts` moves over, no change expected on this side.
...
- 2026-09-26 · B · 1.5.e HTTP hardening done: Fastify `trustProxy`, `; Secure` on the one cookie this API sets (matching `session-cookies.ts`'s own gate), and F12's real session lifetimes (30d idle / 90d absolute consumer, 12h staff) as config instead of a wrong hard-coded 30-minute idle constant — proved sliding, absolute and staff-ceiling behaviour against real Postgres by driving the explicit `now` parameter rather than waiting real days · e5756f3
