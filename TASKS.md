...
| **yourtal-b4** | `platform` (54 tickets) — `apps/api/**`, CI | own `### YT-####` blocks, routed to the recorder |
| **yourtal-28** | `economy` · `data` · `legal` (26) | own blocks, routed |
| **yourtal-0c** | `value` — `services/ledger/**`, `services/voucher/**`, `packages/contracts/src/money/**` | own blocks, routed |
| **yourtal-4d** | `commerce` · `seo` · `watch` | own blocks, routed |
| **yourtal-6c** | **All twelve previously-unowned epics**, assigned by the founder ~17:4x: `infra` · `web` · `value` · `adplatform` · `media` · `watch` · `store` · `commerce` · `merchant` · `risk` · `seo` · `pilot` | not yet stated |
| **yourtal-ca** | **The `review` → `done` sweep** (31 tickets), by founder instruction. Wrote none of the work, so eligible on all of them | none — sends findings to the recorder |
| _yourtal-5f_ | Live, has not stated what it holds. **Ask it; do not infer** | — |
...
⚠️ **Three epics are claimed twice as of ~17:5x, and this table records both claims rather than choosing.** The founder assigned all twelve previously-unowned epics to `yourtal-6c`; meanwhile `yourtal-0c` told the recorder directly that it holds `value`, and `yourtal-4d` that it holds `commerce`, `seo` and `watch` — both before that assignment, both with path lists, and `0c` has already begun YT-0513. **`value`, `commerce`, `seo` and `watch` therefore have two holders each.** This is the founder's to resolve and not the recorder's: picking one would be a coordination decision wearing a bookkeeping one, and the board has already been wrong twice today by inferring ownership rather than asking.
...
**296 tasks** — **78 finished (26%)** · 39 in progress · 175 not started · 4 blocked

**977 engineer-days left of 1195** — **18% of the estimated effort is settled**, against 26% of the task count. Effort counts every `todo`, `doing` and `blocked` task at its FULL estimate, so a half-finished task bills in full. These are ideal engineer-days for one person — divide by real throughput, not by headcount.

Of the 78 finished: **41 independently verified**, 37 awaiting a verifier. A task is only DONE when a session other than the one that did the work has checked it. **The two reviews sampled so far were both wrong**, so that queue is work rather than a formality.

### By phase

| Phase | Done | Review | Doing | Left | Settled |
|---|---|---|---|---|---|
| Phase U · UI first  ◀ NEXT | 13/36 | 15 | 3 | 21d | `████▓▓▓▓░░` 78% |
| Phase −1 · Pilot | 0/13 | 0 | 0 | 36d | `░░░░░░░░░░` 0% |
| Phase 0 · Foundations | 24/127 | 17 | 22 | 248d | `██▓░░░░░░░` 32% |
| Phase 1 · Indonesia MVP | 4/92 | 5 | 14 | 320d | `▓░░░░░░░░░` 10% |
| Phase 2 · Depth | 0/22 | 0 | 0 | 240d | `░░░░░░░░░░` 0% |
| Phase 3 · Marketplace & AU | 0/6 | 0 | 0 | 112d | `░░░░░░░░░░` 0% |

- **Phase U · UI first  ◀ NEXT** — **28 of 36 settled** (13 verified · 15 awaiting a verifier) · 3 in progress · **21d** left · **nothing ready to start now** · 2 more once in-flight dependencies land · 2 blocked outside the graph. Widest gate: **YT-0400** `web` `review` — **16** open tasks downstream.
...
- **Phase 0 · Foundations** — **41 of 127 settled** (24 verified · 17 awaiting a verifier) · 22 in progress · **248d** left · **29 ready to start** · 19 more once in-flight dependencies land. Widest gate: **YT-0039** `platform` `review` — **81** open tasks downstream.
- **Phase 1 · Indonesia MVP** — **9 of 92 settled** (4 verified · 5 awaiting a verifier) · 14 in progress · **320d** left · **4 ready to start** · 14 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0100** `adplatform` `doing` — **68** open tasks downstream.
...
| `economy` | 1/8 | 1 | 2 | **1** +2 | 28d | `█▓▓░░░░░░░` 25% |
| `infra` | 6/32 | 0 | 3 | **12** +3 | 86d | `██░░░░░░░░` 19% |
| `legal` | 1/9 | 1 | 1 | **3** +1 | 31d | `█▓░░░░░░░░` 22% |
| `media` | 0/13 | 1 | 1 | **1** | 67d | `▓░░░░░░░░░` 8% |
| `merchant` | 2/17 | 0 | 5 | — +7 | 66d | `█░░░░░░░░░` 12% |
| `pilot` | 0/11 | 0 | 0 | **1** | 29d | `░░░░░░░░░░` 0% |
| `platform` | 15/55 | 10 | 6 | **11** +7 | 93d | `███▓▓░░░░░` 45% |
| `risk` | 0/15 | 0 | 0 | **4** +2 | 69d | `░░░░░░░░░░` 0% |
| `seo` | 1/5 | 2 | 1 | — | 8d | `██▓▓▓▓░░░░` 60% |
| `store` | 0/10 | 0 | 3 | — +1 | 92d | `░░░░░░░░░░` 0% |
| `value` | 1/27 | 5 | 6 | — +8 | 99d | `▓▓░░░░░░░░` 22% |
| `watch` | 2/8 | 1 | 0 | — +1 | 22d | `███▓░░░░░░` 38% |
| `web` | 11/59 | 16 | 9 | **1** +5 | 127d | `██▓▓▓░░░░░` 46% |
...
- `adplatform` — **1 of 17 settled** (1 verified · 0 awaiting a verifier) · 1 in progress · **93d** left · **2 ready to start** · 2 more once in-flight dependencies land. Widest gate: **YT-0100** `adplatform` `doing` — **68** open tasks downstream.
...
- `economy` — **2 of 8 settled** (1 verified · 1 awaiting a verifier) · 2 in progress · **28d** left · **1 ready to start** · 2 more once in-flight dependencies land. Widest gate: **YT-0048** `economy` `doing` — **24** open tasks downstream.
...
- `legal` — **2 of 9 settled** (1 verified · 1 awaiting a verifier) · 1 in progress · **31d** left · **3 ready to start** · 1 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0012** `legal` `review` — **27** open tasks downstream.
...
- `platform` — **25 of 55 settled** (15 verified · 10 awaiting a verifier) · 6 in progress · **93d** left · **11 ready to start** · 7 more once in-flight dependencies land. Widest gate: **YT-0039** `platform` `review` — **81** open tasks downstream.
- `risk` — **0 of 15 settled** (0 verified · 0 awaiting a verifier) · 0 in progress · **69d** left · **4 ready to start** · 2 more once in-flight dependencies land. Widest gate: **YT-0051** `risk` `todo` — **10** open tasks downstream.
...
- `store` — **0 of 10 settled** (0 verified · 0 awaiting a verifier) · 3 in progress · **92d** left · **nothing ready to start now** · 1 more once in-flight dependencies land. Widest gate: **YT-0130** `store` `doing` — **21** open tasks downstream.
- `value` — **6 of 27 settled** (1 verified · 5 awaiting a verifier) · 6 in progress · **99d** left · **nothing ready to start now** · 8 more once in-flight dependencies land. Widest gate: **YT-0042** `value` `review` — **43** open tasks downstream.
- `watch` — **3 of 8 settled** (2 verified · 1 awaiting a verifier) · 0 in progress · **22d** left · **nothing ready to start now** · 1 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0123** `watch` `todo` — **8** open tasks downstream.
- `web` — **27 of 59 settled** (11 verified · 16 awaiting a verifier) · 9 in progress · **127d** left · **1 ready to start** · 5 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0055** `web` `doing` — **26** open tasks downstream.

### In review (work complete, gate not yet passed)

- **YT-0012** `legal` Counsel-substitution risk acceptance — 4/4 AC ticked
...
- **YT-0540** `platform` Email and password authentication — 4/4 AC ticked
- **YT-0507** `platform` `business` and `kyb_document` resource kinds — 3/3 AC ticked
- **YT-0508** `platform` Promote business shapes into contracts — 2/2 AC ticked
...
- **YT-0039** `platform` Idempotency middleware (TypeScript) — 3/3 AC ticked
- **YT-0553** `platform` API surface for campaign and watch — 12/12 AC ticked
- **YT-0576** `economy` Nobody has defined what makes a settlement decrease "material" — 3/3 AC ticked
- **YT-0042** `value` Ledger transfer API — 6/6 AC ticked
- **YT-0043** `value` Chart of accounts — 7/7 AC ticked
- **YT-0046** `value` Partner funding: point pre-purchase and drawdown — 9/9 AC ticked
- **YT-0512** `web` `apps/web` imports an undeclared package — 5/5 AC ticked
- **YT-0526** `web` Testable HLS fixture for the player — 5/5 AC ticked
- **YT-0550** `web` Player: `Home` does not return the playhead to zero — 1/1 AC ticked
- **YT-0586** `watch` Watch: a seek issued while seeking is silently dropped — 4/4 AC ticked
...
- **YT-0405** `web` Region and locale foundation (AU + ID) — 5/5 AC ticked
...
- **YT-0424** `web` Voucher detail and offline QR — 4/4 AC ticked
...
- **YT-0011** `legal` Red-line register and enforcement · 2d
...
- **YT-0538** `platform` Bot-check, OTP and messaging simulators · 2d
...
- **YT-0514** Idempotency: Go implementation → waiting on YT-0039
- **YT-0549** Seeded user personas for journey and load testing → waiting on YT-0519
- **YT-0559** Contract entries for the campaign, watch and health routes → waiting on YT-0553
