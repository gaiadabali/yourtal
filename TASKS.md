...
| **Phase 5** Watch & earn | B | 🔄 in progress | 4/5 | 18/22 | `████████░░`  82% |
...
- [x] **5.1 A watch session that cannot be farmed** · needs: 1.5, 7.2.c (fake ok) — 🔄 slot 2
  ...
  - [ ] 5.1.d Signed segments (EW-18). Session start returns a per-session manifest URL from `signedSegmentUrl()` in `@yourtal/media` (requested by B; C exports it in 7.2.c). Until then, B calls a local stub with the same signature that returns an unsigned URL. nginx checks the signature (2.1.c). A records the served segments and exposes `deliveryCoverage(sessionId)` (10.4.c), which B's completion calls to flag gaps. — Partially done: `start()` returns `manifestUrl` from a local unsigned stub (`media/segment-url-stub.ts`) and `complete()` calls a `DeliveryCoverageReader` port whose only implementation today is a stub returning `"unknown"` (`delivery-coverage.ts`) — 10.4.c isn't built. Both are already tracked as `(requested by B)`/"B's completion calls it" under 7.2.c and 10.4.c respectively; swap is a two-line change in `watch.module.ts` once either lands.
  - [x] 5.1.e **Check:**
...
- [x] **5.2 Questions during the video, served and scored on the server (F10)** · needs: 5.1
...
- [x] **5.3 Completion grants the reward (EW-06)** · needs: 5.2, 4.4 (fake ok)
