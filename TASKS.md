...
**294 tasks** — **64 finished (22%)** · 50 in progress · 175 not started · 5 blocked

**1004 engineer-days left of 1190** — **16% of the estimated effort is settled**, against 22% of the task count. Effort counts every `todo`, `doing` and `blocked` task at its FULL estimate, so a half-finished task bills in full. These are ideal engineer-days for one person — divide by real throughput, not by headcount.

Of the 64 finished: **40 independently verified**, 24 awaiting a verifier. A task is only DONE when a session other than the one that did the work has checked it. **The two reviews sampled so far were both wrong**, so that queue is work rather than a formality.
...
- **Phase 0 · Foundations** — **36 of 125 settled** (24 verified · 12 awaiting a verifier) · 26 in progress · **252d** left · **28 ready to start** · 18 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0039** `platform` `doing` — **74** open tasks downstream.
...
- `platform` — **22 of 54 settled** (15 verified · 7 awaiting a verifier) · 9 in progress · **99d** left · **10 ready to start** · 7 more once in-flight dependencies land. Widest gate: **YT-0039** `platform` `doing` — **74** open tasks downstream.
