...
| -------------- | --------------------------------------------------------------------------------------- | ----------------------------------------- |
| **yourtal-22** | Orchestrator, 3 agents in git worktrees off `9179ae4`. **(1)** `services/voucher/**` Go only — YT-0150/0151/0152 + the Go half of YT-0039. **(2)** `services/ledger/**`, `docker-compose.yml`, `infra/**`, `.env*`, `services/voucher/Dockerfile` — real HTTP routes over the existing `internal/` libraries. **(3)** `apps/api/src/modules/store/**` (new), `app.module.ts` registration line, `packages/contracts/src/listing/**`, possibly `packages/db/migrations/**` — YT-0130/0131/0132 backend halves, **not** the burn saga YT-0133 | none — barred from `docs/tasks/**` and `TASKS.md`, reports in prose |
