...
| **Phase 5** Watch & earn | B | 🔄 in progress | 4/6 | 20/26 | `████████░░`  77% |
...
| 2 | `yourtal-2` | **5** Watch & earn | 2026-09-26 | 5.1–5.4 ✅. Two agents: C (`yourtal-2`, `phase/5-c`, db `yourtal_s2`) 5.6.a ✅ (0693f67); also fixed an unrelated main regression in `ledger-client.contract.spec.ts` (73767ec, coordinator-requested). 5.6.b ⛔ migration hold (F36 rename not yet on main); 5.6.c ⛔ staging poller stalled at `c9effbe`, 6 releases behind. C stopped here, both Checks blocked on other sessions/infra; resume once either clears. D (`yourtal-p5-b`, `phase/5-d`, db `yourtal_s5b`) 5.5.b–d. 5.5.b's expiring and followed-channel feeds wait on 10.2 and 7.3.f. `phase/5` holds one withdrawn rename commit: never merge it |
...
- [ ] **5.6 Earning proven against the real ledger and staging** · needs: 5.3, 2.1 — 🔄 slot 2
  ...
  - [ ] 5.6.b **Check:** 5.3.b's round trip with `LEDGER_MODE=http` against the running Go ledger service: the pending entry is in the ledger's own tables for the terms' points, and a second complete adds nothing. (5.3.b ran against the fake ledger.) — ⛔ the migration hold: `yourtal_s2`'s `atlas_schema_revisions` still has `20260926153513/14/15` (an earlier, now-superseded local rename), and main still has the ORIGINAL `20260926130000/130100/130200` names, not slot 1's agreed `…160000/160100/160200` (F36) — `pnpm db:migrate` against `yourtal_s2` would hit Atlas's non-linear-history error before applying anything. Re-run once that rename lands on main: repoint those three revision rows to `20260926160000/160100/160200`, then migrate, then drive the real round trip.
  - [ ] 5.6.c **Check:** on staging, a signed manifest URL from session start loads through nginx, and the same URL without its token returns 403. — ⛔ staging is not deploying: `GET https://yourtal.gaiada.com/api/health` still reports revision `c9effbe` (2026-09-26T15:30Z) after 6 further `deploy/production-*` releases published through 16:13Z (including this task's own 5.6.a, `0693f67`), well past the 30-minute grace `staging-drift.yml` allows. The poller (Helios, outside this session's reach — no ssh) appears stalled; cannot be diagnosed or restarted from here. Re-run once staging is confirmed current.
