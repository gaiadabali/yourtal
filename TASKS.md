...
**295 tasks** — **74 finished (25%)** · 42 in progress · 175 not started · 4 blocked

**982 engineer-days left of 1195** — **18% of the estimated effort is settled**, against 25% of the task count. Effort counts every `todo`, `doing` and `blocked` task at its FULL estimate, so a half-finished task bills in full. These are ideal engineer-days for one person — divide by real throughput, not by headcount.

Of the 74 finished: **41 independently verified**, 33 awaiting a verifier. A task is only DONE when a session other than the one that did the work has checked it. **The two reviews sampled so far were both wrong**, so that queue is work rather than a formality.
...
| Phase 0 · Foundations | 24/126 | 16 | 23 | 250d | `██▓░░░░░░░` 32% |
...
- **Phase 0 · Foundations** — **40 of 126 settled** (24 verified · 16 awaiting a verifier) · 23 in progress · **250d** left · **28 ready to start** · 19 more once in-flight dependencies land. Widest gate: **YT-0039** `platform` `review` — **81** open tasks downstream.
...
- `value` — **5 of 27 settled** (1 verified · 4 awaiting a verifier) · 7 in progress · **101d** left · **nothing ready to start now** · 8 more once in-flight dependencies land. Widest gate: **YT-0042** `value` `review` — **43** open tasks downstream.
