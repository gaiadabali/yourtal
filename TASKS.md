...
**294 tasks** — **66 finished (22%)** · 49 in progress · 174 not started · 5 blocked

**995 engineer-days left of 1190** — **16% of the estimated effort is settled**, against 22% of the task count. Effort counts every `todo`, `doing` and `blocked` task at its FULL estimate, so a half-finished task bills in full. These are ideal engineer-days for one person — divide by real throughput, not by headcount.

Of the 66 finished: **41 independently verified**, 25 awaiting a verifier. A task is only DONE when a session other than the one that did the work has checked it. **The two reviews sampled so far were both wrong**, so that queue is work rather than a formality.
...
- `web` — **22 of 58 settled** (11 verified · 11 awaiting a verifier) · 14 in progress · **131d** left · **1 ready to start** · 4 more once in-flight dependencies land · 1 blocked outside the graph. Widest gate: **YT-0055** `web` `doing` — **26** open tasks downstream.
