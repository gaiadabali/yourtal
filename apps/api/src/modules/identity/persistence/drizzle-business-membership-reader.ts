...
} from "./business-membership-reader";
...
export class DrizzleBusinessMembershipReader implements BusinessMembershipReader {
  ...
  async listForUser(userId: string): Promise<readonly BusinessMembershipSummary[]> {
    const rows = await this.db
      .select({
        businessId: businessMembershipsView.businessId,
        role: businessMembershipsView.role,
      })
      .from(businessMembershipsView)
      .where(
        and(
          eq(businessMembershipsView.userId, userId),
          isNotNull(businessMembershipsView.joinedAt),
        ),
      );
    return rows;
  }
