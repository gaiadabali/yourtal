...
describe("buildQuickFeedItemLabel", () => {
  it("states position, total, merchant and title in one sentence", () => {
    expect(
      buildQuickFeedItemLabel({ merchantName: "Toko ABC", title: "Promo Kilat", position: 3, total: 12 }),
    ).toBe("Video 3 dari 12: Toko ABC — Promo Kilat");
  });

  it("handles a feed of exactly one item", () => {
    expect(buildQuickFeedItemLabel({ merchantName: "Toko Solo", title: "Satu-satunya", position: 1, total: 1 })).toBe(
      "Video 1 dari 1: Toko Solo — Satu-satunya",
    );
  });
});
