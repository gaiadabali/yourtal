...
describe("StoreOfferTerms", () => {
  it("shows the minimum spend amount for a minimum_spend policy", () => {
    render(<StoreOfferTerms partialRedemptionPolicy="minimum_spend" minimumSpendIdr={toIdrMinorUnits(100_000)} transferable={false} />);
    expect(screen.getByText(/100\.000/)).toBeInTheDocument();
  });

  it("states transferability plainly", () => {
    render(<StoreOfferTerms partialRedemptionPolicy="balance_carrying" minimumSpendIdr={null} transferable />);
  ...
  it("names the policy label so the terms are identifiable at a glance", () => {
    render(<StoreOfferTerms partialRedemptionPolicy="single_use_forfeit" minimumSpendIdr={null} transferable={false} />);
