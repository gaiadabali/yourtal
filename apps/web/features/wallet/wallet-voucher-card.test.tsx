...
// A voucher's currency is its own field (YT-0513): the AU variants below
// are the IDR fixtures with `currency` (and its amount) swapped to AUD,
// never a prop the caller could set independently of the voucher.
const auExpiringWithinHourVoucherFixture = {
  ...expiringWithinHourVoucherFixture,
  currency: "AUD" as const,
};
const auExpiredVoucherFixture = { ...expiredVoucherFixture, currency: "AUD" as const };

describe("WalletVoucherCard", () => {
  it("links to the voucher's detail page via its title", () => {
    render(
      <WalletVoucherCard voucher={expiringWithinHourVoucherFixture} nowMs={nowMs} locale="id-ID" />,
    );
  ...
  it("shows an already-expired voucher as archived and still fully viewable, never hidden", () => {
    render(<WalletVoucherCard voucher={expiredVoucherFixture} nowMs={nowMs} locale="id-ID" />);
...
describe("WalletVoucherCard (en-AU, YT-0405)", () => {
  it("labels status in English and renders the remaining value in AUD, never a hardcoded Rp", () => {
    render(
      <WalletVoucherCard
        voucher={auExpiringWithinHourVoucherFixture}
        nowMs={nowMs}
        locale="en-AU"
      />,
    );
  ...
  it("shows an archived voucher's end date in English", () => {
    render(<WalletVoucherCard voucher={auExpiredVoucherFixture} nowMs={nowMs} locale="en-AU" />);
