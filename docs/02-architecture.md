...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **YourtalID**           | IdentityProvider seam for email and password authentication. Behind the seam, email/password is current; OIDC and phone OTP are deferred drivers (docs/16 N-3, N-5). Refresh-token rotation planned. |
...
> ⚠️ **Web changes this materially.** Shipping as a PWA means **no Play Integrity and no App Attest** — the strongest control in the native plan is simply unavailable. The compensating stack is in [`08-web-app-and-performance.md`](08-web-app-and-performance.md) §2.1, and the table below is the web version. The primary control available on web is the **CDN segment-log cross-check**, which needs no client cooperation at all. Phone OTP, originally planned as a primary control, is deferred per docs/16 N-3/N-5 and will be re-added when real points are issued.

**Layered, in order of when they fire:**

| Layer                   | Check                                                                                                                                                                                                                                                                                                                        | Blocks                                                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **Registration**        | Email and password, Turnstile, fingerprint (weighted lower on web), IP/ASN reputation, headless/automation detection. Phone OTP is deferred (docs/16 N-5) and will be re-added when real points are issued.                                                                                                                  | Mass account creation                                                                         |
