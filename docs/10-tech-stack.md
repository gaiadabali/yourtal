...
| Auth (identity provider) | **Zitadel** (or Keycloak)                                   | We need to _be_ an OIDC provider for the sister apps. Zitadel is far lighter to operate than Keycloak with the same capability; Keycloak if you have the ops appetite. **Avoid Auth0** — per-MAU pricing is hostile at consumer scale |
