...
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| ID-1  | YourTal Points are a loyalty currency, not e-money, because users cannot buy them, cannot transfer them, and there is no published fixed cash rate. **Expiry, once the fourth leg, is off since 2026-09-25** (F2, F18): built and switchable per region, but absent by default, so this position is weaker than when it was written | BI Reg 10/2025 scope; the distinguishing features of licensed e-money              | **Medium** | If a regulator looks through the structure to substance, the platform is an unlicensed PJP. **This is the single largest legal exposure in the plan**                                                    | 2026-12-19            |
...
11. **No real personal data — Indonesian or Australian — on Helios**, until its location is established by a document rather than by inference. Simulated and internal data only. See below.
    F2 (2026-09-25) accepts Helios for production later. This red line still holds until that document exists.

### What actually enforces each red line — audited 2026-09-21 (YT-0011)

**Two of eleven are enforced. The other nine rely on nobody having built the thing yet**, which is a different guarantee and a weaker one: it expires the moment someone builds it, silently and in good faith.

| #   | Red line                        | Enforced by                                                                                                  | Real?                       |
| --- | ------------------------------- | ------------------------------------------------------------------------------------------------------------ | --------------------------- |
| 1   | No prediction/guessing games    | nothing                                                                                                      | ❌                          |
| 2   | No chance prize without permit  | `packages/jurisdiction` — `prizeDrawsEnabled: false` for ID and AU, schema default `false`, locked by a test | ✅ **and config-proof**     |
| 3   | No cash withdrawal              | `packages/jurisdiction` — `cashOutEnabled: false`, same shape                                                | ✅ **and config-proof**     |
| 4   | No user purchase of points      | nothing forbids it; there is simply no purchase path                                                         | ❌ absence, not enforcement |
| 5   | No forward breakage claim       | one doc comment citing Scoopon in an onboarding page                                                         | ❌                          |
| 6   | No sensitive-category inference | nothing                                                                                                      | ❌                          |
| 7   | No merchant without valid NIB   | a KYB document slot exists; no verification gate found                                                       | ❌                          |
| 8   | No charitable funds held        | `charity` is out of scope until Phase 3                                                                      | ❌ absence                  |
| 9   | No Indonesian launch pre-PSE    | referenced in comments only                                                                                  | ❌                          |
| 10  | No user-level advertiser export | nothing                                                                                                      | ❌                          |
| 11  | No real personal data on Helios | nothing — written today                                                                                      | ❌                          |

**The good news is that the pattern to copy already exists in this repository**, and lines 2 and 3 are a model worth following exactly. `packages/jurisdiction/src/policy-schema.ts` makes the flag **mandatory** rather than optional, so a new jurisdiction cannot be added without answering the question; `policy-data.ts` sets it `false` for both ID and AU; and `policy-data.test.ts` asserts `false` for **every** jurisdiction, so flipping it in configuration alone turns the suite red. That last part is what makes it a control rather than a default — and it is precisely what the YT-0011 criterion means by _"a red line cannot be crossed by configuration alone"_.

**The distinction the table is drawing** is between _forbidden_ and _not yet built_. Red lines 4 and 8 are currently unreachable because the feature does not exist — no purchase path, no charity module. That is safety by absence, and it ends the day someone implements the feature correctly and innocently, with no test failing and no reviewer alerted, because **nothing anywhere states the prohibition in code**. Lines 4 and 3 are also the two that void the counsel-substitution risk acceptance if crossed, which makes the asymmetry worth fixing: one of that pair is enforced and the other is not.

**Schema defaults are deliberately stricter than the live data**, which is the right direction: `minimumAgeYears` defaults to **21** (KUHPerdata Art. 330 contractual capacity) while ID and AU are both set to **18**, and `minimumKycTier` defaults to `enhanced`. A misconfiguration therefore fails toward caution rather than permission.

### Helios residency — accepted as Jakarta on inference, 2026-09-21

**Accepted by founder decision as Jakarta, Indonesia. Confidence: Medium, and the confidence is the point.** The evidence is consistent and entirely indirect: `ipinfo.io` reports Jakarta and `AS47583 Hostinger International Limited`, the reverse hostname is a Hostinger VPS, latency is **14.4 ms** to Bandung and **169.7 ms** to Sydney, which rules Australia out rather than ruling Indonesia in. The server timezone is `Etc/UTC` and tells us nothing.

**Why this is accepted and still does not unlock anything.** For **APP 8** and for Indonesian residency obligations, what matters is a **contractual statement of where data is held** — a hosting agreement or an invoice. Geolocation is evidence about a route, not an undertaking by a provider, and no quantity of it becomes a commitment. So the location is recorded and red line 11 stands until a document supersedes it. **A confirmed location would not by itself lift the line either**: confirming Jakarta makes Australian personal data on Helios a **cross-border disclosure** requiring APP 8 treatment, on the primary market's main path. Knowing costs more than not knowing here.

**What would lift it:** one Hostinger billing or contract page naming the datacentre, plus the APP 8 position decided for Australian data — either it does not go to Helios, or it goes with the disclosure handled. **Country isolation (`docs/02`) is not relaxed to match what the infrastructure happens to be.** The architecture's two-region design stands; Helios is a development box that does not satisfy it, which is a fact about Helios rather than a reason to weaken the design.
