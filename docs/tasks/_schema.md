...
**A claim can be entirely true and attached to the wrong ticket, and re-verification will never catch it.** On 2026-09-21 YT-0140 carried a `⛔` reading *"should not reach `done` while the title still claims an API that does not exist"* — **copy-pasted from YT-0042, where it is correct.** YT-0042 is titled *"Ledger transfer API"* and has no route; YT-0140 is titled *"Voucher issuance and code custody"* and claims no route at all. The paragraph was true, well-argued, and about a different ticket, and it **gated 39 open tickets** until `yourtal-ca` read it against its own subject during the sweep.

This is a distinct failure from a stale claim. A stale claim is false and re-checking it finds that out. **This one is true, so verifying it confirms the paragraph and never asks whether it belongs** — it would survive any amount of re-verification. The check that finds it is not *"is this accurate?"* but *"is this about the ticket it is written on?"*, and the tell is a note whose reasoning never names the ticket it sits in. Found by `yourtal-0c`, who named the shape; the paragraph was written by the recorder, which is how one template reached two tickets.
...
**And the sixth is a GREEN, which is why it was the expensive one.** `yourtal-5f` flipped `keyboard-seek.spec.ts`'s `Home` case from `test.fixme` to `test(` **on three consecutive green full-suite runs**, then measured properly and found it fails **roughly one run in three**. At a 30% failure rate, three greens in a row happen about a third of the time — **their evidence was indistinguishable from luck.**

**For an intermittent symptom, a repeat count is not evidence. Measure a rate.** Three consecutive passes sounds like rigour and is a coin landing heads twice.

**The asymmetry underneath it is the part to keep: a red gets investigated by whoever is blocked by it; a green gets banked by everyone downstream.** Every other entry in this table is a false red, found because it stopped someone working. This one cost two tickets — 5f ticked YT-0550 and called YT-0586 a duplicate on the strength of it — and surfaced only because `yourtal-4d` refused to accept a conclusion about their own ticket that **5f would have accepted from themselves**.
