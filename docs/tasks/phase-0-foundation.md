...
`doing` · P0 · infra · 1d · dep: YT-0516
- [ ] ⚠️ **Never actually ran.** The repository had no remote until 2026-09-20, so this workflow had executed zero times when these boxes were ticked. On its first real execution the job died in `Initialize containers`, before any test
...
- [ ] ⚠️ **This criterion was ticked on reasoning, and the reasoning was wrong.** A service container starts before `actions/checkout`, which the note acknowledged — but the argument that `watchForChanges` covers it fails because **that setting lives in `config.yaml`, which failed to mount for the identical reason**. Docker created a *directory* at `/config/config.yaml`, Cerbos logged `Loading configuration from __default__` and `Found 0 executable policies`, and the healthcheck ran against a directory. **The compensating control was disabled by the fault it was compensating for** — and no amount of reading the workflow would show it, only running it
...
- [ ] **Proved by a green `Integration` run on GitHub**, not by local reasoning. This ticket's whole history is why: every box here was defensible from the file and false in the runner
