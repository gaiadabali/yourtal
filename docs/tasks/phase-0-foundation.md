...
- [x] Red line 4 is enforced in code by the YT-0011 pattern — a mandatory flag, a default that fails toward caution, and a test that fails if the flag is flipped by configuration alone
- [x] **A test fails if the prohibition is removed**, not merely if it is violated, so safety-by-absence cannot be mistaken for enforcement
- [x] Every red line that **voids** the YT-0012 acceptance is enforced to the same standard, **stated as a property** — a criterion naming only red line 4 leaves the next voiding condition to be found the same way
- ✅ **Verified 2026-09-22 by `yourtal-fe`; built by `yourtal-28`, so author and verifier differ. `packages/jurisdiction` is 5 files / 56 tests green, re-run here**
- ✅ **RULING on the question its author raised against themselves: the criterion asked for "a mandatory flag", they shipped `z.literal(false)`, and that EXCEEDS the criterion rather than sidestepping it.** `policy-schema.ts:71` is `userPointPurchaseEnabled: z.literal(false)` against `:34`'s `cashOutEnabled: z.boolean()`. It is stronger on all three things the criterion asked for — mandatory (a required field), failing toward caution (`true` cannot exist), and a configuration flip does not merely fail a test, **it fails to parse**
- ✅ **The distinction they drew between the two red lines is the substance, and it is right.** Red line 3 is *conditional* — *"no cash withdrawal… **until licensed**"* — so a switch is the honest type, because one day a licensing project flips it. Red line 4 is *"No user purchase of points. **Ever.**"* **A boolean models a decision that is not ours to make as a configuration value someone could set.** The literal makes the prohibited state unrepresentable: a record with `true` does not parse, so it cannot reach a running system even if someone writes it, reviews it and ships it
- ✅ **Criterion 3 is satisfied as a property rather than an instance.** `voiding-red-lines.ts` lists the red lines that void YT-0012 paired with the field enforcing each, `field` is typed `keyof JurisdictionPolicy` **so an entry naming a deleted field fails to compile**, and the tests assert over the list rather than over two named fields. **A future voiding red line added without enforcement fails there** — which is what "the property, not the instance" means in practice
- 🚨 **The most valuable thing in this ticket is a test its author found passing for the wrong reason.** Flipping the flag to `true` in `policy-data.ts` did **not** fail the "locked off in every jurisdiction" assertion: the record becomes unparseable, `parseJurisdictionPolicy` falls back to `RESTRICTIVE_FALLBACK_POLICY` **by design**, and the fallback also has the flag off — **so the assertion passed while the jurisdiction had silently lost its entire real policy**
- ✅ **Failing safe is correct; being indistinguishable from correct is not.** The added assertion (`voiding-red-lines.test.ts:44-45`) is that a jurisdiction's live policy is **not byte-identical to the fallback**, which detects a silently substituted record for **any** field rather than only the one under test. Re-sabotaged by its author and it now fails by name: *"ID is byte-identical to the restrictive fallback, which means its own record did not parse"*
- ⚠️ **A package-level hazard beyond this ticket, raised rather than decided: the fail-safe fallback is SILENT.** Any malformed policy record downgrades a jurisdiction to restrictive defaults with nothing reporting it. This test now catches it for ID and AU. **Whether `packages/jurisdiction` should log or throw at load is a design call for its owner**, and `yourtal-28` deliberately did not make it
- ℹ️ `policy-schema.test.ts`'s `VALID_POLICY` gained the field because the schema is strict — **that propagation is the control working**: a new required prohibition cannot be introduced without every policy record answering it, the same property that stops a new jurisdiction being added without answering
