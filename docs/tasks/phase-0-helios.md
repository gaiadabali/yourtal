...
`doing` · P0 · platform · 2d · dep: YT-0535

- [x] Bot check returns pass, fail and expired-token; the **verify step is real code** even when the token is simulated
- [x] OTP simulator issues and burns single-use codes with a real TTL and real velocity limits
- [ ] Codes are retrievable in development through a deliberate, logged, development-only route — never printed into ordinary logs. **The never-logged half is DONE and proved; the ROUTE is not built.** `peekCode(challengeId, nowMs)` reads a separate store that is the only place a raw code lives and appends to a `devAccessLog` recording **that** a code was fetched, never the code. Both members exist **only on `SimulatedOtpDriver`**, so a live driver cannot expose them at compile time. What is missing is an HTTP surface — see the blocker below

**Two of three criteria met and landed as `f3a4e36`; `doing` rather than `review` because the third is genuinely half-built.** It gates 8 tickets including YT-0033.

- ⚠️ **The three boundaries already existed** — `packages/drivers/src/boundaries/{bot-check,otp,messaging}.ts`, wired into `registry.ts` and covered generically by `unhappy-path.test.ts`, introduced in a commit whose message does not mention them and with no dedicated tests. So the claims were **plausible rather than exercised**, which is the state this board keeps finding behind a ticked box
- **The OTP simulator now hashes what it issues.** The challenge stores a SHA-256 `codeHash` and never the plaintext, and `verify()` compares with `timingSafeEqual` over the hash bytes rather than `===`. **A database read must not yield a usable credential** — the same reason `identity.session` and voucher codes are hashed, and an OTP is a credential
- **Three internal outcomes collapse to one refusal at the boundary.** `already_consumed`, `not_found` and `expired` all return `"refused"`, mirroring how `AuthService` collapses `identity.verification_token`'s outcomes into `token_invalid`, **so a caller cannot enumerate.** The granular reason survives internally and is reachable only through `debugClassify` on the simulated driver — never on `OtpDriver`, so a live driver has no member that could leak it
- **Two velocity scopes, never combined:** per-phone 3/15min and gateway-wide 20/15min. One protects a user from a targeted attack, the other protects every user from a broad one, **and a single counter does neither job well.** The proof that they are genuinely independent rather than one counter with two names: fill the global cap across **twenty different numbers**, verify one of phone A's codes, and A becomes issuable again while its *next* issuance is still refused — **by the gateway-wide rule, named in the refusal detail**
- **Everything proved by causing it, not configuring it.** Replay a consumed code → refused. Sit **one millisecond either side of `expiresAtMs`** with the identical code and clock, rather than asserting the TTL constant. Issue three to one number and watch the fourth decline. Issue twenty across twenty numbers, each individually under its own cap, and watch a **twenty-first brand-new number** refused anyway
- **The bot-check criterion's real demand was the adversarial half, and that is what was missing.** A near-miss token, the empty string, and a **valid-shaped token from another boundary's vocabulary** are all rejected rather than defaulting to a pass. *A simulator that returns true for anything token-shaped has tested nothing* — which is what “the verify step is real code even when the token is simulated” actually requires
- **Log absence proved behaviourally and structurally**: a spy on all five `console.*` methods across a full issue → wrong-verify → right-verify → replay flow asserts **zero** calls, and a structural grep proves `otp.ts` contains no `console.`/`Logger` reference at all — so there is no code path that *could* print a code. The same technique YT-0536 used for `Math.random`
- **`packages/drivers` 9 files / 117 tests**, typecheck and lint clean — lint caught real `yt/must-use-result` violations in the new tests, fixed by asserting on the results rather than suppressing

**⛔ BLOCKER on the remaining half, and it is a decision rather than work.** The dev-only retrieval **route** needs `apps/api` to depend on `@yourtal/drivers`, and it does not — confirmed in both `apps/api/package.json` and `pnpm-lock.yaml`. Adding a `workspace:*` dependency requires an install that rewrites the lockfile, and **that was reported rather than run**, correctly: a fresh resolution took every session's `pnpm verify` down at gate 1 last night.

- ⏭️ **The route arguably belongs to YT-0033** (`Phone OTP login flow`, `dep: YT-0540, YT-0538`), which is the ticket that will consume this driver and therefore the one that has to add the dependency anyway. Building the route here would add the dependency for a dev-only endpoint before the feature that needs it exists
- **The design is ready to drop in:** a `GET`, so no `@Idempotent`/`@NotValueMoving` decision, gated to non-production, calling `otpDriver.peekCode(challengeId, Date.now())`, with the controller logging **that** the route was hit — route, challenge id, requester — and never the return value
- ⚠️ **A precedent the route must not copy, found by this work and since fixed:** `AuthService.deliver()` cited this ticket's *“never printed into ordinary logs”* language and then logged the **raw token** through the ordinary logger, guarded only by `nodeEnv !== "production"` — a guard that cannot work here, because `principal.service.ts:55` **throws** in production, so no running instance is ever in production mode. Fixed in `d0c325d` with the same seam shape as `peekCode`. **The lesson for whoever builds this route: a different log level on the same logger is not a different destination**
...
- [x] **Every ref, not just `main`.** `origin` carries `main` and `production`; there are **3 remote `deploy/*` tags** and 5 local `worktree-agent-*` branches. A rewrite that misses one ref leaves the history reachable from that ref
...
- ✅ **THE REPOSITORY WENT PUBLIC AGAIN ON 2026-09-22, BY FOUNDER DECISION, AND THE REWRITE HELD. This ticket's whole warning was about exactly this moment** — *"the danger is not the current state but a future, reasonable-looking decision"* — and the decision came for an unrelated reason: Actions minutes, to end a CI outage
- ✅ **Acceptance evidence, run in the form this ticket demands rather than the tip-scoped form it rejects.** A scan for public IPv4 literals across **`git rev-list --all --remotes --tags` — 232 commits, every ref** — returns only `169.254.169.254` (the AWS link-local metadata address) and `192.0.2.1`–`192.0.2.4` (**RFC 5737 documentation range**, i.e. deliberate placeholders). **Zero real addresses.**
- ✅ **The second ref is covered, which this ticket called *"the single easiest way for this whole exercise to accomplish nothing"*.** `origin/production` is clean, and so are all **four** `deploy/*` tags — `20260920-082910`, `20260920-084351`, `20260920-085836` and `20260921-141343`. Checked by enumerating `git ls-remote` rather than assuming which refs exist
- ⚠️ **What this evidence does and does not establish.** It proves **no public IPv4 literal** survives anywhere reachable. It does **not** re-prove the hostname, OS-version and jump-host removals — those were verified at rewrite time on the mirror (**99 commits carried an identifier before, 0 after**), and that measurement included the negative control this ticket insisted on: **the same scan shown returning non-zero on the pre-rewrite state.** The tip-scoped check reported 0 in both cases and would have passed a rewrite that did nothing
- ⛔ **Two criteria remain genuinely open and neither is about infrastructure addresses.** The recurrence guard — *a check that fails when an infrastructure address appears in a tracked file* — does not exist, so nothing prevents this returning via a runbook written in the wrong repository. And the repository being public now changes what else is worth reviewing: **the board documents unfixed, actionable security findings, and `docs/24`/`docs/25` carry the legal risk acceptance and the decided backing rates.** That is a founder call, not a rewrite, and it is a different question from the one this ticket was filed about