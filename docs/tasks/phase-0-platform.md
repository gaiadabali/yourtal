...
- [ ] ⚠️ **Campaign read routes have NO TESTS AT ALL.** The routes exist and read correctly — `@Get()` and `@Get(":campaignId")`, `@Authorize({ kind: "campaign_view", action: "watch_open" })`, returning `Campaign` whose type cannot express an authoring state. But `apps/api/src/modules/campaign/` contains **no `.test.ts` of any kind** — not for the controller, not for `drizzle-campaign.repository.ts`. The watch half has `watch.controller.test.ts` (14 passing); the campaign half has nothing. **Deliberately left unticked**: this project's own history is controls that were built, tested, and then not used, and a route asserted only by reading it is one step weaker again
...
- ⚠️ **Found by `yourtal-24` 2026-09-20 while verifying rather than reading: the campaign read routes have NO tests at all.** `apps/api/src/modules/campaign/` contains no `.test.ts` of any kind — not for `campaign.controller.ts`, not for `drizzle-campaign.repository.ts`. The watch half has `watch.controller.test.ts` at 14/14; the campaign half has nothing
