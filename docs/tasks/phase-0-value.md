...
- [x] ⚠️ ~~**Confirm against the Xendit contract or sandbox.**~~ — **answered 2026-09-21 from Xendit's public documentation. It was never blocked on a contract or a sandbox; it was blocked on nobody having looked.** Finding: **Xendit expresses IDR in whole Rupiah, not sen.** Evidence, all public and no account required: `POST /payment_requests` types `request_amount` as `number, minimum 0` with **every** IDR example a whole Rupiah figure (`100000`, `50000`, `250000`, `500000`); the **minimum QR transaction is 1 IDR**, which would be Rp 0.01 if the field were sen; and the API reference states **no minor-unit convention for IDR anywhere**. Confidence **high, not certain** — the docs never say it in words, so this is inference from three consistent signals rather than a quoted statement, and the sandbox remains the way to make it certain
...
- [x] If Xendit takes rupiah, the rule shape is wrong: conversion belongs in each **PSP adapter**, not one global constant — **the antecedent is now true, and the rule shape is already right.** `packages/drivers` puts `declaredMinorUnitExponent` on the driver interface, `provider-amount.ts` converts between stored and provider units, and `payments-parity.test.ts` already carries a case titled _"a driver that speaks whole Rupiah while we store sen"_ whose comment reads _"the case the design exists for, not a failure. **Xendit may well be this.**"_ It is. **The prediction was correct and the machinery was built before the answer arrived**
- ❌ **But the default is wrong in the dangerous direction, and this is the actionable finding.** `disbursement.ts:69` reads `IDR: options.declaredMinorUnitExponent?.IDR ?? 2` — so a real Xendit driver constructed **without explicitly passing `{ IDR: 0 }`** inherits our storage exponent and **sends 100× the intended amount**: a Rp 45,000 payout leaves as Rp 4,500,000. The conversion layer is correct; the fallback silently assumes the provider agrees with us, which we now know Xendit does not
- ℹ️ **Recommended shape: give IDR no default at all and make the exponent required per driver**, so a PSP adapter cannot be constructed without stating what its provider speaks. Same lesson as `CHECKPOINT_TOKEN_SECRET`, where the deliberate absence of a default is the control — **a default is the tempting fix and the default is the vulnerability.** `packages/drivers` is not `economy`'s to change; raised rather than edited
...
`doing` · P0 · economy · 5d · dep: YT-0048

**Audited 2026-09-22 against the code rather than started from scratch, and most of criterion 1 was already built.** `services/ledger/internal/pricing` — `price.go`, `engine.go`, `solvency.go` with tests beside each.

- [x] `points_price = (S / B) × demand_multiplier`; multiplier fixed at 1.0 for launch — **met, and built more carefully than the criterion asks.** `PriceInPoints(settlementMinor, backingMicrosPerPoint, demandBps)` computes `ceil(S × 100 × bps / B)` in `big.Int`, **deliberately not `int64`: the intermediate product overflows for a large enough S, and an overflowed price is a NEGATIVE price — a listing that pays the user to take it.** Rounding is always up, because rounding down sells below backing in the same direction every time so the shortfall accumulates rather than averaging out. There is a **floor of 1 point**, since a zero-point listing is an unmetered claim on the reserve. The multiplier is basis points bounded `8_000..12_500` (0.80–1.25) and rejected outside that, with 1.0 as `10_000`
- [ ] Price locked for 15 minutes once shown; never changes between cart and confirm — **nothing exists.** An unfiltered sweep of `services`, `apps/api/src` and `packages/*/src` for a price lock, quote expiry or a locked-until field returns only unrelated hits: auth throttling windows and watch-coverage seconds. **This is the whole of the remaining work on this ticket**, and it is also YT-0133's third criterion, which was correctly deferred here rather than implemented twice
- [ ] Every price change is audit-logged per SKU — **the rate-level audit is real and the per-SKU half records no price.** `ledger.backing_rate` carries `micros_per_point`, `issue_price_micros_per_point`, `effective_from`, **`reason` and `set_by`**, with `Engine.SetRate` / `RateAt(currency, at)` / `History` over it — a genuine audit trail, and `effective_from` makes a **forward-dated** rate change expressible, which is `docs/25` §4's "announce before, never after" in SQL rather than in prose. But that is the **global** rate, not a SKU. The per-SKU trail is `store.listing_price_revision`, which `yourtal-b6` confirmed writes `newPriceInPoints: null` because the store cannot compute a points price by GRANT. **So every price change is logged per SKU except the price**
- ⛔ **The decided rates exist in no database row, and this is the gap that matters.** `ledger.backing_rate` holds **71 rows, all `AUD`, all from `pricing_test`** — zero IDR rows, and the AUD fixtures sit at `600000` micros (0.6 cents/point) against the decided **3 cents**. The founder set `P_issue` 900 sen, `B` (ID) 600 sen and `B` (AU) 3 cents on 2026-09-21 and they live only in `docs/25`. **Same shape as everything else caught this week: decided in a document, absent from the system**
- ✅ **One thing that lines up exactly, and is worth not breaking.** `B` = 600 sen converts to `600_000_000` micros-per-point, and `price_test.go` already states that in its own words — *"backing is 600 sen, and 600 sen expressed in micros is 600_000_000"*. The founder's decision and the engine's test fixtures agree without anyone having coordinated them, because both came from `docs/09` §4.1. **That is luck worth converting into a constraint**: seeding the real rate should assert against `MINOR_UNIT` rather than restate the number
- ℹ️ **Three units are in play and each is correct in its own layer** — `docs/25` and the founder decide in **sen**, `packages/contracts` stores in **minor units**, and the pricing engine takes **micros per point** so a rate finer than one sen is representable. `MicrosPerMinorUnit = 1_000_000` is the only bridge. Worth stating because the 100× class of defect this project has hit twice lives exactly at boundaries like this one
