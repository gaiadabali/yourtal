...
`doing` · P1 · watch · 5d · dep: YT-0120

- [ ] Per-checkpoint signed single-use token; **a spent nonce cannot be spent again, across a restart**
...
- [ ] A seek target arriving while `video.seeking` is still true is coalesced rather than dropped
- [ ] `Home` returns the playhead to within one frame of 0 from a mid-video position
- [ ] `keyboard-seek.spec.ts`'s `Home` case runs as `test`, not `test.fixme`
- [ ] Coverage accounting is unchanged by the fix — a dropped-then-applied seek earns nothing either way
