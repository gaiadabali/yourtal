...
`review` · P1 · watch · 2d · dep: —

- [x] A seek target arriving while `video.seeking` is still true is coalesced rather than dropped
- [x] `Home` returns the playhead to within one frame of 0 from a mid-video position — verified 2026-09-21 against the real MinIO origin, passing on both the `chromium` and `mobile-320` projects
- [x] `keyboard-seek.spec.ts`'s `Home` case runs as `test`, not `test.fixme` — flipped and green **three consecutive runs, 8 passed each** (38.8s / 33.8s / 33.9s), which is the bar YT-0550 set rather than one lucky pass
...
- ⚠️ **Closed from the `web` epic, and this ticket is a duplicate of YT-0550.** One defect, two tickets: `yourtal-54` filed this in `watch` from YT-0412's e2e pass, while YT-0550 — *"Player: `Home` does not return the playhead to zero"* — already existed in `web` for the same race, and carried the component fix. The last two criteria here were closed by YT-0550's work (`yourtal-5f`, the former `yourtal-c8`), not by separate effort in this epic. Recorded rather than quietly ticked, because **a defect recorded twice can be fixed once and still look open** — `yourtal-54`'s own rule from earlier today, arriving back at their own ticket. **Whoever owns `watch` should treat this as a pointer to YT-0550 and consider cutting it**; I have not cut another epic's ticket.
