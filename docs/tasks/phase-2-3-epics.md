...
- ⛔ **Deferral CONFIRMED 2026-09-21, and it is blocked on two things rather than one.** The founder ruled on best recommendation and industry best practice; `yourtal-54` owns this ticket and recommended the deferral stand. Record the reason as **blocked on a deferred dependency AND on the absence of a data layer** — not merely on YT-0543. Three independent grounds, any one sufficient: (1) **there is no BFF seam at all** — 13 of 13 live data sources are stubs, zero `fetch(` in `apps/web/features` (YT-0519, failed verification), and indexable per-district pages need real data to be pages rather than fixtures; (2) **Australia Post licenses the postcode data and the ABS does not**, and SA2 is the correct unit because postcodes are delivery routes rather than polygons — starting before that is settled buys a rewrite; (3) **the cohort floor must live in the aggregation, not the renderer**, because a density map at low coverage re-identifies people, and a privacy property retrofitted into a renderer is one that gets bypassed. Plus the plain one: three `seo` tickets sit at `doing`, nearly finished, and **finishing them is cheaper than starting anything**
...
- ⏭️ **Near-term value, and it depends on the taxonomy (YT-0543) rather than on the map (YT-0544/0545).** If the map is deferred again this should not be — a sequencing argument, recorded as a note because no implementation can tick it. Converted 2026-09-21: flagged by `yourtal-08`'s board scan, judged and proposed by `yourtal-54`, who owns the ticket. **It also contradicted its own header** — the ticket reads `dep: YT-0543` while the box asserted it does not depend on the map. Both are true, taxonomy and map being different tickets, but a criterion needing that footnote to stop being self-contradictory is prose wearing a checkbox. Leaves **one real criterion**, which is the honest count
