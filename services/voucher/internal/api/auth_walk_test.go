...
// 4.10: every /internal/v1 route refuses a call apps/api did not sign. The routes are
// walked from the router itself, so a new route cannot skip this. Mounted as
// cmd/voucher mounts it; no engine is reached, so no database is needed.
func TestEveryRouteRefusesAnUnsignedOrForgedCall(t *testing.T) {
	...
	routes := api.New(logger, nil, nil, nil, nil).Routes()
	...
	if err := chi.Walk(routes, func(method, route string, _ http.Handler, _ ...func(http.Handler) http.Handler) error {
		...
		for name, header := range map[string]string{
			"unsigned":       "",
			"bad signature":  serviceauth.Sign([]byte("not-the-voucher-service-secret-32b"), "api", unique("n"), method, path, body, now),
			"other body":     serviceauth.Sign(secret, "api", unique("n"), method, path, []byte(`{}`), now),
			"other path":     serviceauth.Sign(secret, "api", unique("n"), method, "/internal/v1/wallet/list-x", body, now),
	...
	if walked < 19 {
		t.Fatalf("walked %d routes; the voucher service serves at least 19", walked)
	}
}

var (
	secret  = []byte("test-only-voucher-service-secret-32b")
