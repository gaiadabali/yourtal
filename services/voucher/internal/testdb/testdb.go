...
// running against a real database. YT-0571: several _test.go files here
// hard-coded "postgres://.../yourtal" — a real dev database — as a
// fallback, so a Go test run outside `pnpm --filter @yourtal/voucher-service
// test` wrote fixtures straight into dev data.
//
// A separate copy of the ledger service's identical package, not a shared
// import — the two services are separate Go modules on purpose (docs/13,
// module boundaries enforced twice); a shared test helper would be the
// first thread of the dependency that separation exists to prevent.
package testdb
	...
	if raw == "" {
		t.Fatalf(
			"%s is not set. This suite refuses to guess a database — run it via "+
				"`pnpm --filter @yourtal/voucher-service test`, which creates a fresh "+
	...
	if !strings.HasPrefix(name, testDBPrefix) {
		t.Fatalf(
			"%s names database %q, which is not a %s* database — refusing to run "+
				"against it. Run this suite via `pnpm --filter @yourtal/voucher-service test`, "+
